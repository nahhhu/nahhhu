# ¡Hola! Soy Nahhhu :D

🎓 Técnico en informática | Estudiante de Licenciatura en Sistemas | Futuro Desarrollador Backend  
Interesado por la música, la tecnología, las matemáticas y aprender.

---

## 🚀 Stack de Tecnologías

- **Lenguajes:** Java 21/25 - Spring Boot, JavaScript, HTML, CSS, SQL.  
- **Herramientas:** Intellij, Git, VSCode, Postman y Bruno, Trello, Figma, mi cerebro 🧠  
- **Aprendiendo activamente:** Spring Security, Unit Testing (JUnit 5 & Mockito)
- **Metas de aprendizaje**: Docker, Sring AI.
  
---

## 💡 Proyectos destacados

- [Proyecto final de bootcamp](https://github.com/nahhhu/proyecto-Springboot):
  Desarrollo de una API RESTful profesional para la gestión de inventario de productos electrónicos.
- [💻 Proyecto de Bootcamp FullStack ITBA](https://github.com/00nic/Muebleria-Hermanos-Jota):
  Aplicación web para una mueblería utilizando el stack MERN. Trabajando en equipo junto a 3 compañeros, cumpliendo con sprints y aplicando metodologías ágiles.
- [📌 Proyecto de bootcamp](https://github.com/nahhhu/Preentrega-bootcamp-java):  
  CRUD que aplica los conceptos fundamentales de POO con Java.

---

## 🧩 ¿Dónde practico?

- 🔍 [LeetCode](https://leetcode.com/u/nahhhu/)

---

## 📫 Cómo contactarme

- 📧 Email: nahucordero22@gmail.com  
- 🌐 [LinkedIn](https://www.linkedin.com/in/nahhhu)

---
